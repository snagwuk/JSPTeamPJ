            ...
            HttpServletResponse response) throws ServletException, IOException
    {
        ...
        try
        {
            for (Method method : methods)
            {
                ...
                if (re != null && re.value().equals(command)
                        && re.method().name().equals(tempMethod))
        ...
        else
        {
            try
            {
                throw new RequestNotMatch(command + "  not requestMapping in "
                        + getClass().getName() + "for " + tempMethod);
            }
