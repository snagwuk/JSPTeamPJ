<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!doctype html>
<html lang="en">
<body>


    <!--================Home Banner Area =================-->
    <section class="home_banner_area">
        <div class="banner_inner">
            <div class="container">
                <div class="row">
                    <div class="col-lg-8">
                        <div class="banner_content">
                            <h2>시민과 함께<br /> 사회적 가치를 실현하는 전문 공기업</h2>
                            <p>Health & Beauty. Enjoj your life width 쌍용문화센터</p>
                            <a class="banner_btn" href="applyForm.jsp">온라인 수강신청</a>
                        </div>
                    </div>
                    <div class="col-lg-4">
                        <div class="home_right_box">
                            <div class="home_item">
                                <a href="#none">
                                    <i class="fa fa-clone"></i>
                                    <span class="main_icotxt ">프로그램안내</span>
                                </a>
                            </div>
                            <div class="home_item ">
                                <a href="#none">
                                    <i class=" fa fa-map "></i>
                                    <span class="main_icotxt ">오시는길</span>
                                </a>

                            </div>
                            <div class="home_item ">
                                <a href="#none">
                                    <i class="fa fa-calendar"></i>
                                    <span class="main_icotxt ">수강신청</span>
                                </a>
                            </div>
                            <div class="home_item ">
                                <a href="#none">
                                    <i class=" fa fa-building "></i>
                                    <span class="main_icotxt ">대관신청</span>
                                </a>
                            </div>
                            <div class="home_item ">
                                <a href="#none">
                                    <i class="fa fa-bullhorn "></i>
                                    <span class="main_icotxt ">공지사항</span>
                                </a>
                            </div>
                            <div class="home_item ">
                                <a href="#none">
                                    <i class=" fa fa-envelope-open"></i>
                                    <span class="main_icotxt ">건의사항</span>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!--================End Home Banner Area =================-->
  <!-- Footer Area start  -->
 	 <jsp:include page="/common/footer.jsp" />
    <!-- Footer Area end  -->

   





    <!-- Optional JavaScript -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <script src="js/jquery-3.2.1.min.js "></script>
    <script src="js/popper.js "></script>
    <script src="js/bootstrap.min.js "></script>
    <script src="js/stellar.js "></script>
    <script src="vendors/lightbox/simpleLightbox.min.js "></script>
    <script src="vendors/nice-select/js/jquery.nice-select.min.js "></script>
    <script src="vendors/isotope/imagesloaded.pkgd.min.js "></script>
    <script src="vendors/isotope/isotope-min.js "></script>
    <script src="vendors/owl-carousel/owl.carousel.min.js "></script>
    <script src="js/jquery.ajaxchimp.min.js "></script>
    <script src="vendors/counter-up/jquery.waypoints.min.js "></script>
    <script src="vendors/counter-up/jquery.counterup.min.js "></script>
    <script src="js/mail-script.js "></script>
    <!--gmaps Js-->
    <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCjCGmQ0Uq4exrzdcL6rvxywDDOvfAu6eE "></script>
    <script src="js/gmaps.min.js "></script>
    <script src="js/theme.js "></script>
</body>

</html>